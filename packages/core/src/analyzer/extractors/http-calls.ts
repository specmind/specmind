...
import type { HttpCall, SupportedLanguage } from '../../types/index.js'
import type { FunctionDefinition, ClassDefinition } from '../../types/index.js'
...
export function extractHttpCalls(
  ...
  for (const func of functions) {
    lineToQualifiedName.set(func.location.startLine, func.qualifiedName)
  }
  for (const cls of classes) {
    for (const method of cls.methods) {
      lineToQualifiedName.set(method.location.startLine, method.qualifiedName)
    }
  }
    ...
    if (cursor.nodeType === 'call_expression') {
      ...
      if (calleeNode) {
        ...
        if (isHttpCall(calleeName)) {
          const httpCall = extractHttpCallDetails(node, filePath, lineToQualifiedName)
          if (httpCall) {
...
function extractHttpCallDetails(
  ...
  filePath: string,
  lineToQualifiedName: Map<number, string>
): HttpCall | null {
  const calleeNode = node.childForFieldName('function')
  ...
  let callerQualifiedName = 'global'
  ...
  while (parent) {
    if (
      ...
      if (parent.type === 'function_declaration' || parent.type === 'method_definition') {
        ...
        if (nameNode) {
          callerName = nameNode.text
          const lineNum = parent.startPosition.row + 1
          callerQualifiedName = lineToQualifiedName.get(lineNum) || callerName
          break
        }
      }
      break
  ...
  if (clientType === 'fetch') {
    ...
    if (secondArg && secondArg.type === 'object') {
      ...
      if (methodProp) {
        method = extractStringValue(methodProp).toUpperCase()
      }
    }
